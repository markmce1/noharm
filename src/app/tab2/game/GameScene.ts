...
export default class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        // Add score text
        this.scoreText = this.add.text(16, 16, this.score.toString(), { fontFamily: '"04b19"', fontSize: '32px', color: '#fff' });
    ...
    update() {
        ...
        children.forEach((child) => {
            if (child instanceof Phaser.Physics.Arcade.Sprite) {
                ...
                // @ts-ignore
                if (child.x < birdyX && !this.gameOver && child.texture.key === 'fencea' && !child.scored) {
                    // only check one pipe
                    // @ts-ignore
                    child.scored = true;
                    this.score += 1;
